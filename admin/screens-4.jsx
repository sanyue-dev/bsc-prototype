// admin/screens-4.jsx — 报表中心（antd Tabs + Segmented 重构版）
/* global antd */
...
function genRevSeries(n) { return Array.from({length:n},(_,i) => Math.round(950+170*Math.sin(i*0.38+1.2)+i*1.6)) }
...
function periodKPIs(period, station) {
  ...
  return {totalRev,elecFee,svcFee,discount,totalOrd,avgPerOrd,completeR:94.2,abnormalR:3.1,onlineR:avg(online).toFixed(1)}
}
...
function HorizBar({ label, value, max, color, suffix, secondary }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10 }}>
      <div style={{ width:130, fontSize:12.5, color:ADMIN.textRegular, textAlign:'right', flexShrink:0, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{label}</div>
      ...
      <div style={{ width:70, fontSize:12.5, fontWeight:600, color:ADMIN.textPrimary, fontVariantNumeric:'tabular-nums', flexShrink:0 }}>
        {value}{suffix}
...
function MetricTooltip({ text }) {
  ...
  const handleEnter = () => { if (!ref.current) return; const r=ref.current.getBoundingClientRect(); setPos({x:r.left+r.width/2,y:r.top}) }
  const handleLeave = () => setPos(null)
  const show = pos!==null
  const TIP_W=230
  const clampedX = pos ? Math.min(Math.max(pos.x,TIP_W/2+8),window.innerWidth-TIP_W/2-8) : 0
  const arrowOffset = pos ? pos.x-clampedX : 0
  return (
    <span ref={ref} style={{ display:'inline-flex',alignItems:'center',marginLeft:3,verticalAlign:'middle' }}
      onMouseEnter={handleEnter} onMouseLeave={handleLeave}>
      <span style={{ width:14,height:14,borderRadius:'50%',border:`1px solid ${show?ADMIN.primary:ADMIN.border}`,display:'inline-flex',alignItems:'center',justifyContent:'center',fontSize:9.5,fontWeight:700,color:show?ADMIN.primary:ADMIN.textSecondary,cursor:'default',flexShrink:0,lineHeight:1,background:'#fff',transition:'border-color .15s, color .15s',userSelect:'none' }}>?</span>
      {show && ReactDOM.createPortal(
        <div style={{ position:'fixed',left:clampedX,top:pos.y-8,transform:'translate(-50%,-100%)',background:'#1e2d40',color:'#e8edf3',borderRadius:6,padding:'9px 12px',fontSize:12,lineHeight:1.65,width:TIP_W,zIndex:99999,boxShadow:'0 6px 20px rgba(0,0,0,0.25)',pointerEvents:'none',whiteSpace:'normal' }}>
          {text}
          <div style={{ position:'absolute',top:'100%',left:`calc(50% + ${arrowOffset}px)`,transform:'translateX(-50%)',width:0,height:0,borderLeft:'5px solid transparent',borderRight:'5px solid transparent',borderTop:'5px solid #1e2d40' }}/>
        </div>, document.body
      )}
    </span>
  )
  ...
  return (
    <ElCard padding="16px 20px">
      <div style={{ fontSize:12, color:ADMIN.textSecondary, marginBottom:6, display:'flex', alignItems:'center' }}>
        {label}{tooltip && <MetricTooltip text={tooltip}/>}
      </div>
      <div style={{ fontSize:24, fontWeight:700, color:color||ADMIN.textPrimary, fontVariantNumeric:'tabular-nums', lineHeight:1.2 }}>{value}</div>
...
function ChartTitle({ children, tooltip }) {
  return (
    <div style={{ fontSize:13, fontWeight:600, color:ADMIN.textPrimary, marginBottom:12, display:'flex', alignItems:'center' }}>
      {children}{tooltip && <MetricTooltip text={tooltip}/>}
    </div>
  )
}
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14 }}>
        <ReportKPI label="服务费收入" value={`¥${kpi.svcFee.toLocaleString()}`} color={ADMIN.primary} sub="平台实际收入" tooltip="统计周期内所有已完成订单中收取的服务费总额。服务费是平台的实际收入来源，不含电费。"/>
        <ReportKPI label="净服务费" value={`¥${(kpi.svcFee-kpi.discount).toLocaleString()}`} color={ADMIN.successColor} sub="扣除优惠后" tooltip="服务费收入减去本期全部优惠抵扣后的实际入账金额。"/>
        <ReportKPI label="优惠抵扣" value={`¥${kpi.discount.toLocaleString()}`} sub={`占服务费 ${(kpi.discount/kpi.svcFee*100).toFixed(1)}%`} tooltip="统计周期内优惠券、满减活动等各类折扣的总抵扣金额。"/>
        <ReportKPI label="代收电费" value={`¥${kpi.elecFee.toLocaleString()}`} sub="代收代付，非平台收入" tooltip="向用户收取后转付给电力公司的电费，属于过路款，不计入平台服务费收入。"/>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:14 }}>
        <ElCard padding="18px 20px">
          <ChartTitle tooltip="统计周期内每日平台服务费收入变化趋势。">服务费趋势</ChartTitle>
          <LineChart data={svcChart} height={160} color={ADMIN.primary}/>
        </ElCard>
        <ElCard padding="18px 20px">
          <ChartTitle tooltip="将本期服务费收入拆分为实收净额与优惠抵扣两部分。">收入构成</ChartTitle>
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', paddingTop:8 }}>
            ...
            <Legend items={donutSegs.map(s=>({color:s.color,label:s.label,value:`¥${s.value.toLocaleString()} (${(s.value/(kpi.svcFee||1)*100).toFixed(0)}%)`}))}/>
          </div>
        </ElCard>
      </div>
      <ElCard padding="18px 20px">
        <ChartTitle tooltip="各站点本期已完成订单的收入总额从高到低排列。">站点收入排行</ChartTitle>
        {stationRevData.map((s,i) => (
          <HorizBar key={i} label={s.name} value={`¥${s.rev7.toLocaleString()}`} max={maxRev} color={i<3?ADMIN.primary:'#91c4f9'} suffix="" secondary={`${s.ord7} 单`}/>
        ))}
      </ElCard>
    </div>
  )
...
function OrdersTab({ period, station }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14 }}>
        <ReportKPI label="订单量" value={kpi.totalOrd.toLocaleString()} color={ADMIN.primary} sub="笔" tooltip="统计周期内发起的全部充电订单数。"/>
        <ReportKPI label="完成率" value={`${kpi.completeR}%`} color={ADMIN.successColor} sub="正常结算" tooltip="正常完成结算的订单数 ÷ 本期全部已结束订单数 × 100%。"/>
        <ReportKPI label="平均客单价" value={`¥${kpi.avgPerOrd}`} sub="已完成订单均值" tooltip="本期总收入 ÷ 已完成订单数。"/>
        <ReportKPI label="异常终止率" value={`${kpi.abnormalR}%`} color="#f56c6c" sub="含设备故障" tooltip="因设备故障、断电、网络中断等非用户主动操作导致充电中途终止的订单占比。"/>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:14 }}>
        <ElCard padding="18px 20px">
          <ChartTitle tooltip="以统计周期内每日的订单创建数量绘制柱状图。">订单量趋势</ChartTitle>
          <BarChart data={ordChart} height={160} color={ADMIN.successColor}/>
        </ElCard>
        <ElCard padding="18px 20px">
          <ChartTitle tooltip="将本期所有订单按最终状态分类展示各类占比。">订单状态分布</ChartTitle>
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', paddingTop:8 }}>
            <div style={{ position:'relative', display:'inline-flex', alignItems:'center', justifyContent:'center' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:14 }}>
        <ElCard padding="18px 20px">
          <ChartTitle tooltip="统计本期内按小时分组的订单创建量。">充电高峰时段分布</ChartTitle>
          <BarChart data={HOUR_DATA} height={140} color={ADMIN.primary}/>
        </ElCard>
        <ElCard padding="18px 20px">
          <ChartTitle tooltip="将本期已结束的充电会话按终止原因归类。">充电终止原因</ChartTitle>
          <div style={{ marginTop:10 }}>
            {stopReasons.map((r,i) => <HorizBar key={i} label={r.label} value={r.value} max={100} color={r.color} suffix="%"/>)}
          </div>
        </ElCard>
      </div>
    </div>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14 }}>
        <ReportKPI label="端口平均利用率" value="63.4%" color={ADMIN.primary} sub="充电时长/可用时长" tooltip="统计周期内所有端口实际在充电的时长，占全部端口可用总时长的比例。"/>
        <ReportKPI label="平均充电时长" value="1h 54m" sub="已完成会话" tooltip="本期正常完成的充电会话，从开始到结束的平均时长。"/>
        <ReportKPI label="本期故障工单" value={String(tickets)} color="#f56c6c" sub="已处理 + 待处理" tooltip="本期内产生的故障工单总数。"/>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
        <ElCard padding="18px 20px">
          <ChartTitle tooltip="各站点本期端口实际充电时长占可用总时长的比例。">各站端口利用率</ChartTitle>
          {stationData.map((s,i) => (
        ...
        <ElCard padding="18px 20px">
          <ChartTitle tooltip="统计周期内端口平均利用率的每日变化折线。">端口利用率趋势</ChartTitle>
...
function UsersTab({ period }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14 }}>
        <ReportKPI label="注册用户" value="15,284" sub="累计" tooltip="截至统计截止日，累计通过微信扫码登录过平台的用户总数。"/>
        <ReportKPI label="活跃用户" value={activeUsers.toLocaleString()} color={ADMIN.primary} sub="本期充电 ≥1 次" tooltip="本期内至少完成过 1 次充电的不重复用户数。"/>
        <ReportKPI label="本期新增" value={newUsers.toLocaleString()} color={ADMIN.successColor} sub="首次扫码充电" tooltip="本期内首次完成充电的用户数。"/>
        <ReportKPI label="复购率" value="71.4%" color={ADMIN.primary} sub="本期充电 ≥2 次用户" tooltip="本期充电 2 次及以上的用户数 ÷ 本期活跃用户数 × 100%。"/>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:14 }}>
        <ElCard padding="18px 20px">
          <ChartTitle tooltip="统计周期内每日首次完成充电的用户数变化折线。">新增用户趋势</ChartTitle>
          <LineChart data={usrChart} height={160} color={ADMIN.successColor}/>
        </ElCard>
        <ElCard padding="18px 20px">
          <ChartTitle tooltip="将本期活跃用户按累计消费金额分为四档。">用户消费分层</ChartTitle>
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', paddingTop:8 }}>
            <div style={{ position:'relative', display:'inline-flex', alignItems:'center', justifyContent:'center' }}>
      ...
      <ElCard padding="18px 20px">
        <ChartTitle tooltip="从用户视角统计各时段独立充电用户数。">用户充电高峰时段</ChartTitle>
...
// ── ReportsScreen（antd.Tabs + antd.Segmented） ───────────────────
function ReportsScreen() {
  ...
  const tabBarExtra = (
    <antd.Space style={{ paddingBottom:4 }}>
      <antd.Segmented
        size="small"
        value={period}
        onChange={setPeriod}
      ...
      <antd.Button size="small" icon={<DownloadIcon/>}>导出</antd.Button>
    </antd.Space>
  )
